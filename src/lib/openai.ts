...
export async function tailorFullHtml(
  ...
  if (out.finishReason === 'length' || out.finishReason === 'max_tokens') {
    throw new Error(
      'AI stopped at the output length limit (response truncated). Try a shorter source HTML, or a model with a larger context / higher max output.'
    );
  }

  const parsed = parseTailoredHtmlFromAssistant(out.content);
